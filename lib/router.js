...
var root // path static part
var _started=false
var routeMap={}
var radix=new pStr.Radix
var trigger=function(uri, state){
	var path = uri.substring(root.length)
	var params={}
...
var router={
	...
	go:function(path, replace, state, title){
		path=path||''
		;(replace?h.replaceState:h.pushState).call(h, state || {}, title || '', root + path)
		popstate()
		return this
	},
	depth:function(){
