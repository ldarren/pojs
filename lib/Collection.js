...
function reload(coll){
	...
	for(var i=0,n,id,json,obj; ([n, id]=cache.key(i)); i++){
		if (!n) break
		if (n !== name) continue
		try{
			json=cache.get(n, id)
...
function update(){
	...
	coll.ajax('put', pStr.buildRest(coll.routes.update, coll.rest, model), model, function(err,res){
		if (err) throw err
...
Collection.prototype={
	...
	create:function(model,cb){
		cb=cb||dcb
		var coll=this
		this.ajax('post', pStr.buildRest(this.routes.create, this.rest, model), model, function(err,res){
			if (err) return cb(err)
	...
	read:function(id,cb){
		...
		this.ajax('get', pStr.buildRest(this.routes.read, this.rest, params), params, function(err,res){
			if (err) return cb(err)
			cb(null,set(coll,res))
		}, coll)
	},
	...
	list:function(list,cb){
		...
		this.ajax('get', pStr.buildRest(this.routes.list, this.rest, params), params, function(err,res){
			if (err) return cb(err)
	...
	remove:function(id,cb){
		...
		var params={}
		params[this.idAttr]=id
		this.ajax('delete', pStr.buildRest(this.routes.delete, this.rest, params), params, function(err,res){
			if (err) return cb(err)
