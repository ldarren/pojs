...
Collection.prototype={
	...
	create:function(model,cb){
		cb=cb||dcb
		var coll=this
		Collection.ajax('post',this.routes.create,model,function(err,res){
			if (err) return cb(err)
	...
	read:function(id,cb){
		...
		collection.ajax('get',this.routes.read,params,function(err,res){
			if (err) return cb(err)
			cb(null,set(coll,res))
		})
	},
	...
	list:function(list,cb){
		...
		Collection.ajax('get',this.routes.list,params,function(err,res){
			if (err) return cb(err)
	...
	update:function(model,cb){
		cb=cb||dcb
		var coll=this
		Collection.ajax('put',this.routes.update,model,function(err,res){
			if (err) return cb(err)
			cb(null,set(coll,res))
		})
	},
	remove:function(id,cb){
		...
		var params={}
		params[this.idAttr]=id
		Collection.ajax('delete',this.routes.delete,params,function(err,res){
			if (err) return cb(err)
