...
function populate(coll, id, obj){
	...
	var model = coll.models[id] = Model(obj, this.idAttr)
...
Collection.prototype={
	...
	clear:clear,
	filter: function(cb){
		var list = coll.modelIndex
		var models = []
		for (var i = 0, id; id = list[i]; i++){
			model = get(coll, id)
			if (cb(model, i, id, coll)) models.push(model)
		}
		return models
	},
	find: function(cb){
		...
		for (var i = 0, id; id = list[i]; i++){
			model = get(coll, id)
			if (cb(model, i, id, coll)) return model
		}
	},
	forEach: function(cb){
		var list = coll.modelIndex
		for (var i = 0, id; id = list[i]; i++){
			cb(get(coll, id), i, id, coll)
		}
	},
	sort: function(cb){
		if (!cb) return
